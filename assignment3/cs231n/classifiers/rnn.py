...
class CaptioningRNN(object):
  ...
  def sample(self, features, max_length=30):
    ...
    for i in xrange(max_length):
        prev_word_vec, _ = word_embedding_forward(prev_word_idx, W_embed)
        if self.cell_type == 'rnn':
            next_h, _ = rnn_step_forward(prev_word_vec, prev_h, Wx, Wh, b)
            prev_h = next_h
        elif self.cell_type == 'lstm':
            next_h, next_c, _ = lstm_step_forward(prev_word_vec, prev_h, prev_c, Wx, Wh, b)
            prev_h = next_h
            prev_c = next_c
        else:
            raise ValueError('Invalid cell_type "%s"' % self.cell_type)
