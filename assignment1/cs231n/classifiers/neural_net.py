...
class TwoLayerNet(object):
  ...
  def loss(self, X, y=None, reg=0.0):
    ...
    # shape (N, C).                                                             #
    #############################################################################
    l2 = X.dot(W1) + b1 # NOTE: element-wise addition of bias
    ReLU = np.maximum(0, l2)
    ...
    grads['b2'] = np.sum(dscores, axis = 0)
      # equivalent to np.ones_like(ReLU.T).dot(dscores)
      # bias need not be regularizaed due to element-wise addition of bias

    dReLU = dscores.dot(W2.T)
    dl2 = (ReLU > 0) * dReLU
    grads['W1'] = (X.T).dot(dl2) + reg * W1
    grads['b1'] = np.sum(dl2, axis = 0)
      # equivalent to np.ones_like(X.T).dot(dl2)
      # bias need not be regularizaed due to element-wise addition of bias
    #############################################################################
    #                              END OF YOUR CODE                             #
    #############################################################################
